...
| `@deepseek-ai/dsh-tool-skill-manage` | `manage_skills` | `ctx.tools`、`ctx.skills`、`ctx.skillPreferences for enable and disable` | `tool/call`、`tool/result` | - | 启用状态的写入经过 ctx.skillPreferences，即 Skills 页面写入的同一偏好；skill 目录会在下一步反映它们。 |
...
List installed skills with whether each is enabled, or enable or disable skills by name. A disabled skill leaves the skill catalog and cannot be loaded. Changes apply from the next step. Use scope "project" to change a skill only for the current project; the default changes it for every project.
...
```json
{
  ...
  "properties": {
    ...
    "names": {
      "type": "array",
      "description": "Exact skill names to enable or disable.",
      "items": {
        "type": "string"
      }
    },
    "scope": {
      "type": "string",
      "description": "global (default): every project. project: only the current project.",
      "enum": [
        "global",
        "project"
      ]
...
来源：[`packages/skill/tool-skill-manage/src/index.ts`](../packages/skill/tool-skill-manage/src/index.ts)

启用状态的写入经过 ctx.skillPreferences，即 Skills 页面写入的同一偏好；skill 目录会在下一步反映它们。
