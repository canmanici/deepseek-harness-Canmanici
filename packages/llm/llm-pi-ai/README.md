...
### OpenCode session routing

Routes serving OpenCode Zen (`opencode`) or OpenCode Go (`opencode-go`) send the loop's session id as `x-opencode-session` on every request, which OpenCode Go requires for routing and prompt caching; a profile `headers` entry of the same name overrides it.
