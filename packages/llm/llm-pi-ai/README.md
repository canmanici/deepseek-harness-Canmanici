...
| `api` | catalog protocol | Protocol of every model on the route whose entry names none |
| `baseURL` | catalog endpoint | Endpoint of every model on the route whose entry names none |
...
| `sessionHeader` | absent | Header carrying each request's conversation id; replaces a same-named `headers` entry |
...
A gateway that routes or caches by conversation requires the caller to name one; OpenCode Go refuses every request without `x-opencode-session`. `sessionHeader` names the header and the adapter writes the request's session id into it, so one conversation presents one id across its turns, resumes, compactions, and retries, and a new conversation or fork presents a fresh one. Every wire protocol and both route kinds — a catalog route reusing pi-ai's provider and a route this package builds — send it. The field replaces a same-named `headers` entry, because a fixed value cannot do a per-conversation id's job, while that static entry still covers a request naming no session id; Harness attribution keeps reserved names.
...
A profile's `models` list replaces the route's installed catalog rather than extending it; each entry defaults its unset fields from the installed model of the same id, so narrowing a route to two models, correcting one capacity, or adding a model newer than the installed catalog are one-line edits. An entry may name its own `api` and `baseURL`, which is how a model the catalog does not describe joins a route whose models disagree about their protocols: the entry's values win over the route's, so one route serves several wire formats instead of one route per protocol. `modelOverrides` reshapes individual installed-catalog models without that cost — correct one model, keep the other thirty-seven — and is refused when set beside a `models` list, on a hand-declared route, or naming a model the catalog does not describe, because a silently unchanged model would be a typo someone hunts for later.
...
- **A live interrogation asks one endpoint of a catalog route** — the one most of its models use, so a model served only by another endpoint must be added by hand.
- **A catalog route's model-level protocol must be one its installed provider implements** — a catalog route reuses pi-ai's own provider for dispatch, so a protocol that provider does not implement fails the request with pi-ai's message rather than being refused where it is written; the protocols a hand-declared route may name are the ones this build serves.
