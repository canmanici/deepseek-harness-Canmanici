...
describe('PiAiAdapter provider routing', () => {
  ...
  it('sends the conversation header from the request session id, replacing a same-named entry', async () => {
    const server = await mockServer([{ events: textEvents }, { events: textEvents }])
    const ctx = await harness(server.url, {
      sessionHeader: 'x-opencode-session',
      headers: { 'x-opencode-session': 'static-fallback', 'x-other': 'kept' },
    })

    await assemble(ctx, { model: 'deepseek-flash', messages: [], sessionId: 'session-abc' as never })
    expect(server.headers[0]?.['x-opencode-session']).toBe('session-abc')
    // Only the same-named entry is replaced; unrelated deployment headers stay.
    expect(server.headers[0]?.['x-other']).toBe('kept')

    // A request naming no conversation keeps the static entry, so a route
    // whose gateway requires the header stays routable without one.
    await assemble(ctx, { model: 'deepseek-flash', messages: [] })
    expect(server.headers[1]?.['x-opencode-session']).toBe('static-fallback')
  })

  it('leaves the conversation header off a route that configures none', async () => {
    const server = await mockServer([{ events: textEvents }])
    const ctx = await harness(server.url)
  ...
  it('keeps Harness attribution ahead of a conversation header that collides with it', async () => {
    const server = await mockServer([{ events: textEvents }])
    const ctx = await harness(server.url, { sessionHeader: 'user-agent' })
...
describe('provider profile lifecycle', () => {
  ...
  it.each([
    ['', /has an empty sessionHeader/],
    ['bad name', /sessionHeader "bad name" is not a valid HTTP field name/],
  ] as const)('rejects sessionHeader %j', (name, message) => {
    expect(() => resolveProfiles({ openai: { sessionHeader: name } })).toThrow(message)
    expect(resolveProfiles({ openai: { sessionHeader: 'x-opencode-session' } }).get('openai')?.sessionHeader)
  ...
  it('rejects unsupported or unresolved image input before provider I/O', async () => {
    ...
    // A text-only catalog model refuses the image on its own declaration.
    await expect(drain({
      provider: 'deepseek',
      model: 'deepseek-v4-pro',
      messages: [createUserMessage({
