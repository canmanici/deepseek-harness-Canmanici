...
- **启用过滤器。** 通过 `ctx.skills.registerFilter(...)` 注册的过滤器为每次查找解析一个谓词；`list`、`snapshot` 和 `get` 会省略被任一过滤器禁用的合并胜出项，同名的低优先级 skill 不会替代它。过滤器拒绝时读取也会被拒绝。`ctx.skills.inventory(...)` 为管理界面列出每个胜出项及其 `enabled` 和禁用它的过滤器名称。
