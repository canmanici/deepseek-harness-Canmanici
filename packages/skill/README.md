...
The skill family lets agents and users discover and load reusable task instructions only when needed. Use `skill/` to combine catalogs and expose one instruction set per name; choose `skill-filesystem` for local directories, `skill-sources` for GitHub, archive, or URL sources, `skill-marketplace` to search public skill marketplaces, `skill-preferences` for per-skill on/off, `skill-badge` for the official badge, and `skill-office` for Office workflows. Add `tool-skill` when models should receive a sorted, durable session catalog, load full instructions through the `skill` tool, or accept direct `/name` invocation. Different sources produce the same model-visible format, and model access requires at least one source.
...
| [`skill-preferences/`](skill-preferences/README.md) | Stores global and per-project skill enablement and enforces it as a registry filter | `ctx.skillPreferences`; registers on `ctx.skills` |
| [`skill-marketplace/`](skill-marketplace/README.md) | Searches public skill marketplaces: GitHub repositories, claude-plugins.dev, SkillsMP, and skills.sh | `ctx.skillMarketplace` |
| [`skill-sources/`](skill-sources/README.md) | Syncs skills from GitHub repositories, ZIP archives, and skill-file URLs, starting with the Anthropic public repository | `ctx.skillSources`; registers on `ctx.skills` |
...
| [`tool-skill-manage/`](tool-skill-manage/README.md) | Lets agents and subagents list skills and switch them on or off through `manage_skills` | registers on `ctx.tools` |
